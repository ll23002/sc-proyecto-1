  ...
  <q-page class="q-pa-md">
    ...
    <q-card class="themed-card">
      <q-card-section>
        ...
        <!-- Formulario pequeño para crear transacción manualmente -->
            ...
            <q-card flat class="bg-grey-9 q-pa-sm" style="border-radius:8px">
              <div class="row items-center q-col-gutter-sm">
                ...
                <div class="col-6 col-md-2">
                  <q-input v-model.number="manual.monto" label="Monto" type="number" dense outlined dark />
                </div>
                <div class="col-6 col-md-1">
                  <q-select v-model="manual.moneda" :options="monedaOptions" label="Moneda" dense outlined dark emit-value map-options />
                </div>
                <div class="col-12 col-md-2 text-right">
                  <q-btn color="positive" label="Agregar Manual" @click="crearManual" :loading="creatingManual" />
                </div>
              </div>
            </q-card>
          </div>
    ...
    <q-dialog v-model="showDialog" persistent maximized transition-show="slide-up" transition-hide="slide-down">
      <q-card class="dialog-card bg-grey-10 text-white">
        <q-toolbar class="bg-primary text-white">
          ...
          <q-btn flat label="Guardar Asiento" icon="save" @click="guardarAsiento" :loading="saving" />
        </q-toolbar>
...
<div class="bg-grey-10 q-pa-sm rounded-borders text-caption text-italic">
  "{{ transaccionSeleccionada?.clasificacion?.justificacion || 'Sin sugerencia de IA disponible.' }}"
</div>
            </q-card>
          </div>
          ...
          <div class="col-12 col-md-8">
            ...
            <div class="row q-col-gutter-sm q-mb-md bg-grey-9 q-pa-sm rounded-borders">
              ...
              <div class="col-6 col-sm-2">
                <q-input
                  ...
                  label="Debe" type="number" dense outlined dark
                  @update:model-value="val => { if(val > 0) nuevaLinea.haber = 0 }"
                />
              </div>
              <div class="col-6 col-sm-2">
                <q-input
                  v-model.number="nuevaLinea.haber"
                  label="Haber" type="number" dense outlined dark
                  @update:model-value="val => { if(val > 0) nuevaLinea.debe = 0 }"
                />
              </div>
              <div class="col-12 text-right">
                <q-btn color="secondary" label="Agregar Línea" size="sm" icon="add" @click="agregarLinea" />
              </div>
            </div>
            ...
            <q-markup-table flat dark class="bg-transparent">
              ...
              <tbody>
                <tr v-for="(linea, index) in asientoDetalles" :key="index">
                  ...
                  <td class="text-right text-positive">{{ linea.debe > 0 ? formatCurrency(linea.debe) : '-' }}</td>
                  <td class="text-right text-negative">{{ linea.haber > 0 ? formatCurrency(linea.haber) : '-' }}</td>
                  <td class="text-center">
                    <q-btn flat round icon="delete" color="red" size="sm" @click="asientoDetalles.splice(index, 1)" />
                  </td>
                </tr>
            ...
            <q-banner v-if="!esBalanceado" dense class="bg-warning text-black q-mt-md rounded-borders">
              <template v-slot:avatar><q-icon name="warning" /></template>
              El asiento no cuadra. Diferencia: {{ formatCurrency(Math.abs(totalDebe - totalHaber)) }}
            </q-banner>
...
const manual = ref({ fecha: new Date().toISOString().slice(0,10), descripcion: '', monto: 0, moneda: 'USD' })
...
const crearManual = async () => {
  ...
  try {
    ...
    // reset form
    manual.value = { fecha: new Date().toISOString().slice(0,10), descripcion: '', monto: 0, moneda: 'USD' }
  } catch (e) {
...
const totalDebe = computed(() => asientoDetalles.value.reduce((sum, item) => sum + Number(item.debe || 0), 0))
const totalHaber = computed(() => asientoDetalles.value.reduce((sum, item) => sum + Number(item.haber || 0), 0))
const esBalanceado = computed(() => Math.abs(totalDebe.value - totalHaber.value) < 0.01 && asientoDetalles.value.length > 0)
const balanceClass = computed(() => esBalanceado.value ? 'text-positive' : 'text-warning')

const formatCurrency = (val) => new Intl.NumberFormat('es-SV', { style: 'currency', currency: 'USD' }).format(val)
...
const cargarDatos = async () => {
  loading.value = true
  try {
    const [txRes, ctaRes] = await Promise.all([
...
const abrirDialogoContabilizar = (tx) => {
  ...
  const cuentaBancoId = cuentas.value.find(c => c.nombre_cuenta.toLowerCase().includes('banco'))?.id || null
...
<style scoped>
.themed-card { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); }
.themed-table { background: transparent; }
